...
def main() -> None:
    ...
    statement = "INSERT INTO status (date, subject_id, project, is_uploaded, is_sorted) VALUES "
    ...
    if response[1]:
        print(f"Error inserting values into `status` table ob dbhub: {response[1].get('error', 'Unknown error')}")
    else:

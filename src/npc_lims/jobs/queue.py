...
def check_capsule_exists(capsule_or_pipeline_name: str) -> None:
    ...
    if capsule_or_pipeline_name not in codeocean.MODEL_CAPSULE_PIPELINE_MAPPING:
        raise codeocean.ModelCapsuleMappingError(f'No corresponding capsule for {capsule_or_pipeline_name}')

def is_session_capsule_in_queue(session: str | npc_session.SessionRecord, capsule_or_pipeline_name: str) -> bool:
...
def add_to_queue(session_id: str | npc_session.SessionRecord, capsule_or_pipeline_name: str) -> None:

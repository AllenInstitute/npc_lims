...
S3_PROBE_TARGET_DB_PATH = upath.UPath('s3://aind-scratch-data/arjun.sridhar/probe_targeting/dr_master.db')
...
def get_probe_insertion_info(probe_insertion_default: dict[str, Any], metadata: dict[str, str | int]) -> dict[str, Any]:
    ...
    for letter in probe_letters:
        ...
        probe_insertion_default['probe_insertions'][f'probe{letter}']['hole'] = metadata[f'Probe{letter}']
    
    probe_insertion_default['probe_insertions']['implant'] = metadata['implant']
    return probe_insertion_default

@functools.cache
def get_probe_insertion_metadata(session: str | npc_session.SessionRecord) -> dict[str, Any]:
    """
    ...
    cursor = target_db_connection.execute(f"SELECT * FROM session_metadata sm WHERE sm.session = '{session.subject}_{session.date}'")
    ...
    probe_insertions_default = {'probe_insertions': {'probeA': {}, 'probeB': {}, 'probeC': {}, 'probeD': {}, 'probeE': {}, 'probeF': {}, 
                                             'implant': ''}}

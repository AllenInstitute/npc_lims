...
def get_training_sqlite_paths() -> tuple[upath.UPath, ...]:
    ...
    """
    return tuple(path.with_suffix('.sqlite') for path in get_training_spreadsheet_paths())
...
def update_training_dbs() -> None:
    ...
    >>> update_training_dbs()
    """
    for spreadsheet, sqlite in zip(get_training_spreadsheet_paths(), get_training_sqlite_paths()):
...
def excel_to_sqlite(
    ...
    for sheet in wb.sheetnames:
        ...
        query = 'CREATE TABLE ' + repr(str(slugify(sheet))) + '(ID INTEGER PRIMARY KEY AUTOINCREMENT'
        ...
        for i, col in enumerate(ws):
            ...
            for idx, col in enumerate(col):
                if idx in duplicate_column_idx:
                    continue
                tuprow.append(str(col.value).strip()) if str(col.value).strip() != 'None' else tuprow.append('')
